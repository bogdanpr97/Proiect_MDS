<?php
    ...
    if($_SERVER['REQUEST_METHOD'] != 'POST') {
        ...
        if(!isset($_POST['c_id']) || !isset($_POST['mesaj'])) {
            ...
            $sqlVerificare = "select * from comentarii_articole where c_id = ? and u_id = ? ;";
            if($stmt = $conn->prepare($sqlVerificare)) {
                ...
                if($result->num_rows == 0) {
                    ...
                    $sqlUpdate = "update comentarii_articole set c_text = ? , c_data = concat('Editat la: ', now()) where c_id = ? and u_id = ? ;";
                    if($stmt2 = $conn->prepare($sqlUpdate)) {
                        $stmt2->bind_param("sii", $mesaj, $cId, $_SESSION['uid']);
                        $stmt2->execute();
                        if($stmt2->affected_rows == 1) {
                            $stmt2->close();
                            $file = fopen("comentariiArticoleLog.txt", "a+");
